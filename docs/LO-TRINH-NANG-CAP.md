...
| 4.5 | Join dữ liệu thời tiết | ⚠ **Tiền đề sai — cần đổi nguồn.** Kiểm tra project `Du_bao_thoi_tiet` (ref `bdttccztjtrcaztjgkot`): **KHÔNG chứa dữ liệu thời tiết** — thực ra là hệ tài liệu/RAG/validation GMP (documents, chunks, embeddings, AI queries…). → Phương án thay: hút thời tiết Hà Nội từ **Open-Meteo** (free, không cần key) qua 1 workflow n8n → bảng `thoi_tiet_ngoai` → view tương quan RH trong/ngoài. Chờ xác nhận toạ độ + hướng tiếp cận. |
