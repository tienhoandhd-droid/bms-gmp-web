...
| 3.7 | Nâng cấp tab **Xu hướng GMP** (tùy chọn + biểu đồ mạnh + AI chỉ gợi ý) | ✅ đã làm | **(a) Độ phân giải** 30 phút / 1 giờ / theo ngày — thêm nhánh `PHUT` cho 3 RPC (`rpc_lay_chuoi_xu_huong_v2`, `rpc_chuoi_xu_huong_da_sensor`, `rpc_chuoi_gia_tri_phong`); khung 24h mặc định 30′ (48 điểm). Ghi nhớ lựa chọn (localStorage). **(b) Biểu đồ mạnh hơn:** kéo–thu phóng (dataZoom) + xuất **PNG** (ECharts saveAsImage); RoomBandChart thêm **dải P5–P95 + trung vị P50** (từ `chan_doan`) và **baseline 30 ngày ± σ** (tính trong `rpc_chuoi_gia_tri_phong`). **(c) AI chỉ đọc & gợi ý:** đổi nhãn nút → "AI gợi ý đọc biểu đồ", card → "Gợi ý đọc biểu đồ (AI hỗ trợ)" + disclaimer "AI chỉ đọc số liệu và gợi ý — con số do SQL tính, kết luận GMP do IPC/QA". Build + smoke-test (Playwright) OK: 5 canvas, chip độ phân giải hiện, 0 lỗi console. **(d) Bảng dữ liệu thô + đánh giá cơ bản (tất định):** card mới TRƯỚC phần AI/QA — bảng chi tiết mỗi bucket (TB/Min/Max/P5/P50/P95/GHD/GHT/trạng thái) + đánh giá cơ bản (TB, σ, %trong giới hạn, số điểm OOS, xu hướng, nền 30n); cấp Khu/AHU/Tổng hiện bảng %đạt/OOS/DQ theo thời gian. **(e) Sửa lỗi giới hạn:** payload AI (`gia_tri_thuc_3`) bị rỗng sau khi đổi shape `roomBandsMulti` → AI mất GHD/GHT thật → bịa giới hạn; đã sửa đọc `.series`. Giới hạn nay lấy đúng theo từng phòng trong DB (bảng thô hiển thị tất định). **Còn lại (deferred):** overlay so sánh kỳ trước; siết prompt WF7 chỉ dùng giới hạn được cấp. |
