...
export function hasHourlyHistory(points = []) {
  return normalizeHourlyPoints(points).some(p => p.avg != null);
}
export function chooseSensor(sensors = []) {
  const available = sensors.filter(s => hasHourlyHistory(s.stats?.hourly8));
...
export function missingSnapshot(room, { sourceInterrupted = false, freshnessMinutes = 120 } = {}) {
  ...
  if (room.duLieuCu) return { label: 'Chưa thể đánh giá kỳ hiện tại', detail: `${previous} Biểu đồ 8 giờ gần nhất vẫn hiển thị nếu còn chuỗi số liệu.` };
