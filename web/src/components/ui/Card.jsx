...
export function SectionTitle({ icon: Icon, children, hint }) { return <h3 className="text-sm font-semibold flex items-center gap-2" style={{ color: "var(--text-strong)" }}><Icon className="w-4 h-4" style={{ color: "var(--primary)" }} strokeWidth={1.8} />{children}{hint && <span className="text-[11px] font-normal text-muted">— {hint}</span>}</h3>; }
...
export function HeaderChip({ children, ring = "ring-line" }) { return <div className={`flex items-center gap-2.5 rounded-2xl bg-surface px-4 ring-1 ${ring} h-[50px]`} style={cardShadow}>{children}</div>; }
