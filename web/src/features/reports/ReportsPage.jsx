    ...
    <div className="space-y-5">
      ...
      <Card className="p-6"><SectionTitle icon={Mail} hint="báo cáo quản trị — kỳ liền trước">Gửi lại báo cáo (email)</SectionTitle>
        ...
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          ...
          <button type="button" onClick={guiBu} disabled={guiTT === "DANG_GUI" || !wf5Url} aria-disabled={!wf5Url || undefined}
            className={`min-h-11 text-xs font-medium rounded-lg px-4 py-2 text-white flex items-center gap-1.5 ${guiTT === "DANG_GUI" ? "opacity-60 cursor-wait" : !wf5Url ? "opacity-50 cursor-not-allowed" : ""}`}
          ...
          <button type="button" onClick={() => window.print()} className="min-h-11 text-xs font-medium rounded-lg px-4 py-2 text-body ring-1 ring-line bg-surface hover:bg-subtle flex items-center gap-1.5"><Printer className="w-3.5 h-3.5" strokeWidth={1.8} /> In hoặc lưu PDF</button>
        </div>
        {guiTT && guiTT !== "DANG_GUI" && (guiTT.ok
          ? <p role="status" className="text-xs text-success font-medium mt-3">✓ {guiTT.message || "Đã nhận yêu cầu — báo cáo sẽ được tạo và gửi email trong vài phút."}</p>
          : <p role="alert" className="text-xs text-danger font-medium mt-3">✗ Không gửi được yêu cầu ({guiTT.error === "CHUA_CAU_HINH_WEBHOOK" ? "chưa cấu hình điểm gửi báo cáo — liên hệ quản trị hệ thống" : guiTT.error}). Thử lại hoặc báo IT.</p>)}
        <p className="text-[12px] text-muted mt-3">Danh sách người nhận quản lý ở tab <b>Thông báo</b>; chưa kích hoạt ai thì gửi về địa chỉ dự phòng trong mục Thông báo → Địa chỉ hệ thống. File PDF/HTML đồng thời được lưu vào kho hồ sơ nội bộ.</p>
        <details className="mt-4 rounded-lg bg-subtle px-3.5 py-3 text-[13px] text-body ring-1 ring-line">
          <summary className="cursor-pointer font-medium text-strong">Cách lập báo cáo</summary>
          <p className="mt-2 leading-relaxed text-muted">Nhận định, số liệu, biểu đồ, OOS và độ đầy đủ dữ liệu giữ nguyên theo phạm vi đã chọn ở tab Xu hướng.</p>
        </details>
      </Card>
      <Card className="p-6"><SectionTitle icon={ClipboardCheck} hint="tổng hợp từ tab Xu hướng">Nhận định hỗ trợ</SectionTitle>
        {ai ? <div className="rounded-lg ring-1 ring-success-line p-5 text-sm leading-relaxed text-body mt-4" style={{ background: "var(--bg-subtle)" }}><p className="text-[12px] text-muted mb-2">{ai.scope} · {ai.sensor} · {ai.range} · lập lúc {ai.time}</p><AiSections text={ai.text} /></div> : <div className="rounded-lg ring-1 ring-line bg-subtle p-5 text-sm text-muted mt-4">Chưa có nhận định cho phạm vi đang chọn. Vào tab <b className="text-body">Xu hướng</b> để lập nhận định.</div>}
        {aiRows && aiRows.length > 0 && (
          <div className="mt-5">
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto pr-1">{aiRows.map((r, i) => (
              <div key={`${r.ts || r.thoi_gian || ""}-${r.scope || ""}-${i}`} className="rounded-lg bg-subtle ring-1 ring-line/70 p-3.5">
