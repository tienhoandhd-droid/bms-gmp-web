...
const RoomCard = React.memo(function RoomCard({ room, cfg, onDetail, onIncident, incident }) {
  ...
  return (
    <Card className="p-5 transition hover:-translate-y-0.5">
        ...
        <div className="mt-3 rounded-2xl bg-subtle ring-1 ring-line/70 overflow-hidden">
          ...
          {room.sensors.map((s) => { const st = sensorStats(room.id, s, room._isLive); const lvl = st.khongCoDL ? -1 : ((s._live && s._live.level != null) ? s._live.level : sensorLevel(st, cfg)); const noDL = lvl < 0; const dotCls = noDL ? "bg-subtle" : LEVELS[lvl].dot; const lblMuc = st.khongCoDL ? "Chưa có dữ liệu" : (noDL ? "Cảm biến đứng hình" : LEVELS[lvl].label); return (
            <div key={s.k} className="grid grid-cols-5 items-center px-3 py-2 text-[12px] border-b border-line/50 last:border-0">
              <span className="flex items-center gap-1.5 text-body font-medium">{s.k}<span title={lblMuc} aria-label={lblMuc} className={`inline-flex items-center justify-center w-3.5 h-3.5 rounded-full text-[8px] leading-none font-bold text-white ${dotCls}`}>{levelGlyph(lvl)}</span></span>
              {noDL ? <span className="col-span-4 text-center text-[11px] text-muted italic">{st.khongCoDL ? "chưa có dữ liệu" : "cảm biến đứng hình — số đo không dùng được"}</span> : (<>
              <span className="text-center tabular-nums font-semibold" style={{ color: "var(--text-strong)" }}>{st.cur}<span className="text-[11px] text-muted">{SENSOR_META[s.k].unit}</span></span>
      ...
      {!room.noData && (() => { const oos8 = roomHourlyOOS(room); const tong8 = oos8.reduce((a, h) => a + (h.oos || 0), 0); return <div className="mt-3"><div className="flex items-center justify-between"><span className="text-[10px] uppercase tracking-wider text-muted font-medium">Điểm OOS theo giờ — 8h</span>{oos8.length > 0 && tong8 === 0 && <span className="text-[10px] text-success font-medium">0 điểm OOS · đạt</span>}</div>{oos8.length === 0 ? <p className="text-[11px] text-muted italic py-3 text-center">chưa có dữ liệu 8h</p> : <OosMiniBars data={oos8} h={70} />}</div>; })()}
      {room.note && <p className="mt-3 text-[11px] text-muted bg-info-soft/60 ring-1 ring-info-line rounded-xl px-3 py-2">📝 {room.note}</p>}
      <div className="mt-3 flex gap-2">
        ...
        {incident ? <button onClick={() => onIncident(room)} className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium text-danger bg-danger-soft hover:bg-danger-soft rounded-xl py-2 ring-1 ring-danger-line transition" title={`Sự cố ${incident.id} · ${incident.status}`}><AlertOctagon className="w-3.5 h-3.5" strokeWidth={1.8} /> Sự cố {incident.id} <ChevronRight className="w-3.5 h-3.5" strokeWidth={1.8} /></button>
          : failing ? <span className="flex-1 flex items-center justify-center gap-1.5 text-[11px] text-warning bg-warning-soft rounded-xl py-2 ring-1 ring-warning-line"><AlertTriangle className="w-3.5 h-3.5" strokeWidth={1.8} /> Không đạt — chưa mở sự cố</span> : null}
      </div>
    </Card>
  );
      ...
      <div className="w-full max-w-2xl rounded-3xl bg-surface ring-1 ring-line overflow-hidden max-h-[88vh] overflow-y-auto" style={{ boxShadow: "0 30px 80px -20px rgba(30,58,86,0.5)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-4">{room.noData ? <p className="text-warning text-sm">Phòng đang thiếu dữ liệu — không có cảm biến hoạt động.</p> : room.sensors.map((s) => { const st = sensorStats(room.id, s, room._isLive); const noDL = st.khongCoDL; const pts = st.hourly8 || []; const mean = pts.length ? +(pts.reduce((a, p) => a + (p.avg ?? 0), 0) / pts.length).toFixed(1) : null; const unit = SENSOR_META[s.k].unit; return (
          <div key={s.k} className="rounded-2xl bg-subtle ring-1 ring-line/70 p-4">
            <div className="flex items-center justify-between mb-2"><p className="text-sm font-semibold" style={{ color: "var(--text-strong)" }}>{SENSOR_META[s.k].label} ({s.k})</p><p className="text-[11px] text-muted">Giới hạn: {s.min != null ? `≥ ${s.min}` : "—"}{s.max != null ? ` · ≤ ${s.max}` : ""} {unit}</p></div>
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 mb-2 text-center">{[["Hiện tại", `${st.cur ?? "—"} ${unit}`], ["TB 1h", `${st.avg1h ?? "—"}`], ["TB 8h", mean == null ? "—" : `${mean}`], ["OOS 1h", st.oos1h == null ? "—" : `${st.oos1h}/60`], ["OOS 10′ cuối", st.err10 == null ? "—" : `${st.err10}/10`]].map(([k, v]) => <div key={k} className="rounded-xl bg-surface ring-1 ring-line py-1.5"><p className="text-[11px] uppercase text-muted font-semibold leading-tight">{k}</p><p className="text-[13px] font-semibold tabular-nums" style={{ color: "var(--text-strong)" }}>{v}</p></div>)}</div>
      ...
      <div className="w-full max-w-lg rounded-3xl bg-surface ring-1 ring-line overflow-hidden max-h-[85vh] flex flex-col" style={{ boxShadow: "0 30px 80px -20px rgba(30,58,86,0.5)" }} onClick={(e) => e.stopPropagation()}>
        <div className="px-6 pt-5 pb-4 flex items-start justify-between" style={{ background: "var(--bg-subtle)" }}>
          <div className="flex items-start gap-3">
            ...
            <div><h2 className="text-base font-semibold" style={{ color: "var(--text-strong)" }}>{META.title}</h2><p className="text-[11px] text-muted mt-0.5 max-w-xs">{META.desc}</p></div>
          </div>
              ...
              <div className="space-y-2">
                {incidents.map((i) => { const laP1 = i.priority === "P1"; return (
                  <button key={i.id} onClick={() => onPickIncident(i)} className={`w-full text-left rounded-2xl ring-1 border-l-[6px] px-4 py-3 transition duration-150 flex items-center justify-between gap-3 ${laP1 ? "ring-danger-line border-danger bg-danger-soft/30 hover:ring-danger-line hover:bg-danger-soft/60" : "ring-warning-line border-warning bg-warning-soft/30 hover:ring-warning-line hover:bg-warning-soft/60"}`}>
                    <div className="min-w-0"><div className="flex items-center gap-2"><span className="text-[14px] font-semibold" style={{ color: "var(--text-strong)" }}>{i.id}</span><span className="text-[11px] px-2 py-0.5 rounded-full font-bold text-white bg-danger-solid">Nghiêm trọng</span>{!laP1 && <span className="ml-1 text-[10px] text-muted">quan trọng</span>}</div><p className="text-[12px] text-body mt-0.5 truncate">{i.room} · {i.sensor || "—"} · {i.status}</p></div>
                ...
                {rooms.map((r) => { const comp = roomCompliance(r); const lvl = roomLevel(r, cfg); const lm = lvl < 0 ? null : LEVELS[lvl]; return (
                  <button key={r.id} onClick={() => onPickRoom(r)} className="w-full text-left rounded-2xl ring-1 ring-line hover:ring-success-line hover:bg-success-soft/40 px-4 py-3 transition flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-muted mt-0.5 truncate">{r.id} · Khu {r.area} · {r.ahu}</p>
                    ...
                    <div className="text-right shrink-0">{comp == null ? <span className="text-[11px] text-muted font-semibold">— %</span> : <p className={`text-xl font-light tabular-nums ${comp < 80 ? "text-danger" : "text-success"}`}>{comp}%</p>}<ChevronRight className="w-4 h-4 text-muted ml-auto mt-0.5" strokeWidth={1.8} /></div>
                  </button>
                ); })}
              </div>
            )
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        {canManage ? <button onClick={() => { if (editId && soThayDoi > 0 && !window.confirm("Bỏ các thay đổi chưa lưu?")) return; setOpen((o) => !o); setEditId(null); setDraft(null); }} className="text-xs font-medium text-white rounded-xl px-3.5 py-2 flex items-center gap-1.5" style={{ backgroundColor: "var(--danger-solid)" }}><Plus className="w-3.5 h-3.5" strokeWidth={2} /> Thêm phòng</button> : <span className="text-[11px] text-muted">Cần quyền QA/Quản trị để chỉnh sửa</span>}
      </div>

      {open && canManage && (
        <div className="mt-4 rounded-2xl bg-info-soft/60 ring-1 ring-info-line p-4 space-y-3">
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
            <div className="flex flex-col gap-1"><label className="text-[10px] uppercase text-muted font-semibold">Mã phòng</label><input className={inp} value={f.id} onChange={(e) => setF({ ...f, id: e.target.value })} placeholder="C1.R09" /></div>
            <div className="flex flex-col gap-1 col-span-2"><label className="text-[10px] uppercase text-muted font-semibold">Tên phòng</label><input className={inp} value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} placeholder="Phòng cân" /></div>
            <div className="flex flex-col gap-1"><label className="text-[10px] uppercase text-muted font-semibold">Khu</label><select className={inp} value={f.area} onChange={(e) => setF({ ...f, area: e.target.value })}>{AREAS.map((a) => <option key={a}>{a}</option>)}</select></div>
            <div className="flex flex-col gap-1"><label className="text-[10px] uppercase text-muted font-semibold">AHU</label><select className={inp} value={f.ahu} onChange={(e) => setF({ ...f, ahu: e.target.value })}>{AHUS.map((a) => <option key={a}>{a}</option>)}</select></div>
            <div className="flex flex-col gap-1"><label className="text-[10px] uppercase text-muted font-semibold">Mức ưu tiên</label><select className={inp} value={f.priority} onChange={(e) => setF({ ...f, priority: e.target.value })}><option value="P1">Mức 1</option><option value="P2">Mức 2</option><option value="P3">Mức 3</option></select></div>
          </div>
          <div className="rounded-xl bg-surface ring-1 ring-line p-3">
            <p className="text-[10px] uppercase text-muted font-semibold mb-2">Chọn loại cảm biến & giới hạn (min – max)</p>
      ...
      <div className="flex items-center gap-2 mt-4 flex-wrap">
        ...
        <span className="text-[11px] text-muted ml-auto tabular-nums">{roomsHienThi.length}/{rooms.length} phòng</span>
      </div>
      <div className="overflow-x-auto mt-3">
        <table className="w-full text-[13px]">
          <thead><tr className="text-muted text-left text-[11px] uppercase tracking-wider">{["Mã", "Tên", "Khu", "AHU", "Ưu tiên", "Loại DL", "Mức cảnh báo", ""].map((h) => <th key={h} className="py-2.5 pr-4 font-semibold">{h}</th>)}</tr></thead>
          <tbody>
            {roomsHienThi.length === 0 ? <tr><td colSpan={8} className="py-6 text-center text-[12px] text-muted">Không có phòng khớp bộ lọc{locKhu !== "ALL" ? ` · Khu ${locKhu}` : ""}{locAhu !== "ALL" ? ` · ${locAhu}` : ""}{qTim.trim() ? ` · "${qTim.trim()}"` : ""}. <button onClick={() => { setLocKhu("ALL"); setLocAhu("ALL"); setQTim(""); }} className="text-success font-semibold underline">Bỏ lọc</button></td></tr> : roomsHienThi.map((r) => { const lvl = roomLevel(r, cfg); const lm = lvl < 0 ? null : LEVELS[lvl]; return (
              <tr key={r.id} className="border-t border-line hover:bg-info-soft/40 transition">
                ...
                <td className="py-2 pr-4">{lm ? <span className={`text-[11px] px-2 py-0.5 rounded-full ${lm.bg} ${lm.txt}`}>{lm.label}</span> : <span className="text-[11px] text-warning">Mất DL</span>}</td>
      ...
      {editing && canManage && draft && (
        <div className="mt-4 rounded-2xl bg-success-soft/50 ring-1 ring-success-line p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold" style={{ color: "var(--text-strong)" }}>Sửa phòng & cảm biến — {editing.id}{soThayDoi > 0 && <span className="ml-2 align-middle text-[10px] font-semibold text-warning bg-warning-soft ring-1 ring-warning-line rounded-full px-2 py-0.5">{soThayDoi} thay đổi chưa lưu</span>}</p>
            <button onClick={dongSua} className="text-muted hover:text-body"><X className="w-4 h-4" /></button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
            <div className="flex flex-col gap-1 col-span-2"><label className="text-[10px] uppercase text-muted font-semibold">Tên phòng</label><input className={inp} value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} /></div>
            <div className="flex flex-col gap-1"><label className="text-[10px] uppercase text-muted font-semibold">Khu</label><select className={inp} value={draft.area} onChange={(e) => setDraft({ ...draft, area: e.target.value })}>{AREAS.map((a) => <option key={a}>{a}</option>)}</select></div>
            <div className="flex flex-col gap-1"><label className="text-[10px] uppercase text-muted font-semibold">AHU</label><select className={inp} value={draft.ahu} onChange={(e) => setDraft({ ...draft, ahu: e.target.value })}>{[...new Set([draft.ahu, ...AHUS])].filter(Boolean).map((a) => <option key={a}>{a}</option>)}</select></div>
            <div className="flex flex-col gap-1"><label className="text-[10px] uppercase text-muted font-semibold">Mức ưu tiên</label><select className={inp} value={draft.priority} onChange={(e) => setDraft({ ...draft, priority: e.target.value })}><option value="P1">Mức 1</option><option value="P2">Mức 2</option><option value="P3">Mức 3</option></select></div>
            <div className="flex flex-col gap-1"><label className="text-[10px] uppercase text-muted font-semibold">Ghi chú</label><input className={inp} value={draft.note} onChange={(e) => setDraft({ ...draft, note: e.target.value })} placeholder="(tuỳ chọn)" /></div>
          </div>
          {!editing.noData && (
            <div className="space-y-2 mt-3">
              {draft.sensors.map((s) => (
                <div key={s.k} className="rounded-xl bg-surface ring-1 ring-line px-3 py-2 flex items-center gap-2 text-[12px] flex-wrap">
                  ...
                  <button onClick={() => setDraft((d) => ({ ...d, sensors: d.sensors.filter((x) => x.k !== s.k) }))} className="ml-auto text-danger hover:text-danger text-[11px] flex items-center gap-1"><Trash2 className="w-3.5 h-3.5" /> bỏ</button>
                </div>
              ))}
              {["DP", "RH", "T"].filter((k) => !draft.sensors.some((s) => s.k === k)).length > 0 && (
                <div className="flex items-center gap-2 pt-1"><span className="text-[11px] text-muted">Thêm cảm biến:</span>{["DP", "RH", "T"].filter((k) => !draft.sensors.some((s) => s.k === k)).map((k) => <button key={k} onClick={() => setDraft((d) => ({ ...d, sensors: [...d.sensors, { k, ...SENSOR_DEFAULT[k] }] }))} className="text-[11px] rounded-lg px-2 py-1 ring-1 ring-success-line text-success bg-success-soft hover:bg-success-soft flex items-center gap-1"><Plus className="w-3 h-3" strokeWidth={2} /> {SENSOR_META[k].label}</button>)}</div>
              )}
            </div>
          )}
          <div className="flex items-center justify-between flex-wrap gap-2 mt-4">
            <p className="text-[11px] text-muted max-w-md">Thay đổi chỉ ghi vào hệ thống khi bấm <b>Lưu</b>. Giới hạn là <b>mốc so sánh gốc</b> — sau khi lưu, KPI, mức cảnh báo, thẻ phòng và báo cáo đều tính theo giá trị mới.</p>
      ...
      <p className="text-[11px] text-muted mt-3">Thay đổi sau khi <b>Lưu</b> cập nhật ngay KPI, thẻ phòng và được ghi vào <b>lịch sử thay đổi cấu hình</b> (tab Nhật ký &amp; SOP).</p>
    </Card>
  );
}
