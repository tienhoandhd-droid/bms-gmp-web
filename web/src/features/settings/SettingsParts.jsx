      ...
      {!user ? (
        ...
        <div className="mt-4 space-y-3 max-w-sm">
          <input type="password" value={mkCu} onChange={(e) => setMkCu(e.target.value)} placeholder="Mật khẩu hiện tại"
            ...
            className="w-full rounded-2xl bg-subtle ring-1 ring-line px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-success-line" />
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: "rgba(30,58,86,0.28)", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div className="w-full max-w-sm rounded-3xl bg-surface ring-1 ring-line overflow-hidden" style={{ boxShadow: "0 30px 80px -20px rgba(30,58,86,0.5)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-3">
          <input type="password" value={mkCu} onChange={(e) => setMkCu(e.target.value)} placeholder="Mật khẩu hiện tại" autoComplete="current-password" className="w-full rounded-2xl bg-subtle ring-1 ring-line px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-success-line" />
          <div className="h-px bg-subtle" />
          <input type="password" value={mk1} onChange={(e) => setMk1(e.target.value)} placeholder="Mật khẩu mới" autoComplete="new-password" className="w-full rounded-2xl bg-subtle ring-1 ring-line px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-success-line" />
          <input type="password" value={mk2} onChange={(e) => setMk2(e.target.value)} placeholder="Nhập lại mật khẩu mới" autoComplete="new-password" onKeyDown={(e) => e.key === "Enter" && doi()} className="w-full rounded-2xl bg-subtle ring-1 ring-line px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-success-line" />
          {loi && <p className="text-[12px] text-danger">{loi}</p>}
          {ok && <p className="text-[12px] text-success">Đã đổi mật khẩu thành công.</p>}
...
function TaiKhoanCard({ isLive, actor }) {
  ...
  return (
    <Card className="p-6">
      ...
      <p className="text-[12px] text-muted mt-2">Mỗi tài khoản chỉ <b>xem</b> dữ liệu của các khu được tích — <b>chặn ngay tại máy chủ</b> (mọi tab: Tổng quan · Sự cố · Chênh áp · Xu hướng GMP; kể cả gọi API trực tiếp cũng không lấy được khu khác). Tổng/toàn hệ với tài khoản giới hạn = gộp đúng các khu được xem. <b>Quản trị</b> luôn xem tất cả. Tạo tài khoản đăng nhập mới thực hiện ở Supabase; tại đây gán vai trò & khu — có hiệu lực ngay lần tải dữ liệu kế tiếp.</p>
...
function ChuoiHashCard({ isLive }) {
  ...
  return (
    <Card className="p-6">
      ...
      {kq && (
        <div className={`mt-3 rounded-2xl px-4 py-3 text-[13px] ${kq.ok ? "bg-success-soft text-success ring-1 ring-success-line" : "bg-danger-soft text-danger ring-1 ring-danger-line"}`}>
          {kq.ok ? "✓ " : "⚠ "}{kq.thong_bao}
        </div>
      )}
    </Card>
