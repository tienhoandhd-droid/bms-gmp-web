-- Fix historical cohorts: action-token retention must not erase delivered emails.
-- Evidence: service-written outbox + SMTP accepted + role-matched action link.
-- Token presence in the archived link is structural only; it is never reactivated.
...
BEGIN
  ...
  SELECT e.id,e.gui_luc,e.du_lieu->>'vai_tro' AS role,
         m[1]::bigint AS incident_id,m[2] AS action
  FROM legacy_mails e CROSS JOIN LATERAL regexp_matches(
    replace(e.noi_dung_html,'&amp;','&'),
    '[?&]sc=([0-9]+)&act=([a-z_]+)&token=[^"&<> ]+','g') m
), legacy_actions(role,action) AS (VALUES
  ('IPC','ipc_nhan_viec'),('IPC','ipc_bao_co_dien'),('IPC','ipc_binh_thuong'),('IPC','ipc_da_khac_phuc'),('IPC','ipc_vang'),
  ('MEP','mep_tiep_nhan'),('MEP','mep_cho_xu_ly'),('MEP','mep_khong_xu_ly_duoc'),('MEP','mep_xu_ly_xong'),('MEP','mep_vang'),
  ...
  FROM legacy_links l JOIN legacy_actions a USING(role,action)
  JOIN scoped_incidents si ON si.ma_su_co=l.incident_id
