-- =============================================================================
-- 20260824a — PHÂN LOẠI ĐÚNG CA FMS TRẢ RỖNG TOÀN HỆ
-- =============================================================================
...
--      WF1_PIPELINE_ERROR : workflow thu dữ liệu/rollup giờ lỗi; chênh áp trực tiếp
--                           vẫn có thể dùng nếu mạch phút còn tươi.
--      FMS_DATA_LOSS   : FMS kết nối được nhưng trả rỗng/thiếu điểm đo.
--      FMS_UNREACHABLE : không đăng nhập/gọi được FMS, timeout hoặc WF1 lấy 0 phòng.
--
...
AS $function$
  ...
  phan_loai AS (
    SELECT co.*,
      CASE
        WHEN do_thu_that_bai OR co_login_timeout THEN 'FMS_UNREACHABLE'
        WHEN do_fms_rong_toan_he OR do_bucket_rong_toan_he THEN 'FMS_DATA_LOSS'
        WHEN do_wf1_mat_nhat_ky OR do_wf1_loi OR do_mach_gio THEN 'WF1_PIPELINE_ERROR'
        ELSE 'NORMAL'
  ...
  SELECT jsonb_build_object(
    ...
    'tom_tat', CASE
      WHEN ma_trang_thai = 'WF1_PIPELINE_ERROR'
        THEN 'Workflow thu dữ liệu giờ đang lỗi hoặc quá hạn. Chênh áp trực tiếp vẫn có thể dùng nếu mạch dữ liệu phút còn tươi; kiểm WF1 trên n8n và chạy lại/backfill khi cần.'
      WHEN ma_trang_thai = 'FMS_UNREACHABLE'
        THEN 'Không kết nối hoặc đăng nhập được FMS. Kiểm tài khoản, mạng, endpoint FMS và execution WF1 gần nhất.'
      WHEN ma_trang_thai = 'FMS_DATA_LOSS'
        THEN 'FMS kết nối được nhưng trả rỗng điểm đo: WF1 xử lý '||COALESCE(wf1_phong::text,'?')||' phòng, '||COALESCE(wf1_phong_rong::text,'?')||' phòng rỗng, 0 điểm đo. Tạm dừng kết luận đạt/không đạt cho tới khi FMS có dữ liệu lại.'
...
AS $function$
  ...
  SELECT jsonb_build_object(
    ...
    'tom_tat', CASE
                 ...
                 WHEN (SELECT qua_han FROM tre)
                 THEN 'Workflow thu dữ liệu giờ đang quá hạn. Chênh áp trực tiếp vẫn có thể dùng nếu mạch dữ liệu phút còn tươi; kiểm WF1 và backfill khi cần.'
...
UPDATE public.cau_hinh SET value = '20260824a' WHERE key = 'phien_ban_db';
