...
// Widget GIÁM SÁT SỨC KHỎE (data freshness) — chỉ hiện ở chế độ LIVE.
// Bịt điểm hở lớn nhất của BMS: nếu WF1/FMS ngừng, dữ liệu "đứng" mà không ai biết.
function SucKhoeChip({ sk }) {
  if (!sk) return null;
  const mat = sk.matDuLieu;
  const treGio = sk.treGio;
  const moTaTre = treGio == null ? "chưa có dữ liệu"
    : treGio < 1 ? "cập nhật < 1 giờ trước"
    : `cập nhật ${treGio.toFixed(treGio < 10 ? 1 : 0)} giờ trước`;
  const mau = mat ? { ring: "ring-rose-200", dot: "bg-rose-500", txt: "text-rose-600", nhan: "MẤT DỮ LIỆU" }
                  : { ring: "ring-teal-200", dot: "bg-teal-400", txt: "text-teal-600", nhan: "Dữ liệu mới" };
  const tip = `Bucket mới nhất: ${sk.bucketMoiNhat ? new Date(sk.bucketMoiNhat).toLocaleString("vi-VN") : "—"}`
    + ` · Ngưỡng trễ: ${sk.nguongGio ?? "—"}h`
    + (sk.lanChayCuoi ? ` · WF1 lần cuối: ${sk.lanChayCuoi.trangThai || "?"}` : "")
    + ` · Sự cố mở: ${sk.suCoDangMo} (CRIT ${sk.soCritical}/WARN ${sk.soWarning})`;
  return (
    <div className={`flex items-center gap-2.5 rounded-2xl bg-white px-4 ring-1 ${mau.ring} h-[50px]`} style={cardShadow} title={tip}>
      <span className={`w-2.5 h-2.5 rounded-full ${mau.dot} ${mat ? "animate-pulse" : ""}`} />
      <div className="leading-tight">
        <p className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold flex items-center gap-1"><Activity className="w-3 h-3" strokeWidth={1.8} /> Sức khỏe dữ liệu</p>
        <p className={`text-xs font-semibold ${mau.txt}`}>{mau.nhan} · <span className="text-slate-500 font-medium">{moTaTre}</span></p>
      </div>
    </div>
  );
}
...
function LoginModal({ onClose, onLogin, isLive }) {
  const [email, setEmail] = useState(USERS[0].email);
  const [emailLive, setEmailLive] = useState("");
  const [daGui, setDaGui] = useState(false);
  const [dangGui, setDangGui] = useState(false);
  const [loi, setLoi] = useState("");
  const guiLink = async () => {
    if (!emailLive.includes("@")) { setLoi("Nhập email hợp lệ."); return; }
    setDangGui(true); setLoi("");
    const { error } = await guiMagicLink(emailLive.trim());
    setDangGui(false);
    if (error) setLoi(error.message || "Không gửi được link."); else setDaGui(true);
  };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(30,58,86,0.28)", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div className="w-full max-w-md rounded-3xl bg-white ring-1 ring-slate-200 overflow-hidden" style={{ boxShadow: "0 30px 80px -20px rgba(30,58,86,0.5)" }} onClick={(e) => e.stopPropagation()}>
        <div className="px-6 pt-6 pb-4 flex items-center gap-3" style={{ background: "linear-gradient(135deg,#E6F4F1,#fff)" }}><div className="rounded-2xl bg-white p-2.5 ring-1 ring-teal-100"><LogIn className="w-5 h-5" style={{ color: COLOR.teal }} strokeWidth={1.8} /></div><div><h2 className="text-base font-semibold" style={{ color: COLOR.navy }}>Đăng nhập</h2><p className="text-[11px] text-slate-500">{isLive ? "Gửi link đăng nhập qua email" : "Chọn tài khoản — phân quyền theo vai trò"}</p></div></div>
        {isLive ? (
          <div className="px-6 py-5 space-y-4">
            {daGui ? (
              <div className="rounded-2xl bg-teal-50 ring-1 ring-teal-100 px-4 py-4 text-sm text-slate-700">✓ Đã gửi link đăng nhập tới <b>{emailLive}</b>. Mở email và bấm vào link để vào hệ thống.</div>
            ) : (<>
              <input type="email" value={emailLive} onChange={(e) => setEmailLive(e.target.value)} placeholder="email@cpc1hn.vn" className="w-full rounded-2xl bg-slate-50 ring-1 ring-slate-200 px-4 py-3 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-teal-300" />
              {loi && <p className="text-[12px] text-rose-600">{loi}</p>}
              <button disabled={dangGui} onClick={guiLink} className="w-full text-sm font-semibold text-white rounded-2xl py-3 disabled:opacity-60" style={{ backgroundColor: COLOR.teal }}>{dangGui ? "Đang gửi…" : "Gửi link đăng nhập"}</button>
              <p className="text-[11px] text-slate-400 leading-relaxed">Tài khoản phải có trong hệ thống (bảng người dùng). Vai trò được xác định theo email.</p>
            </>)}
          </div>
        ) : (
          <div className="px-6 py-5 space-y-4"><select value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-2xl bg-slate-50 ring-1 ring-slate-200 px-4 py-3 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-teal-300">{USERS.map((u) => <option key={u.email} value={u.email}>{u.name} — {ROLE_VI[u.role]}</option>)}</select><button onClick={() => { onLogin(USERS.find((u) => u.email === email)); onClose(); }} className="w-full text-sm font-semibold text-white rounded-2xl py-3" style={{ backgroundColor: COLOR.teal }}>Đăng nhập</button><p className="text-[11px] text-slate-400 leading-relaxed"><b>IPC</b> tiếp nhận/kiểm tra · <b>Cơ điện</b> xử lý · <b>QA</b> xác nhận khắc phục · <b>Trực HSL</b> điều phối · <b>Quản trị</b> toàn quyền.</p></div>
        )}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [boChanTam, setBoChanTam] = useState(false);   // tạm bỏ chặn để xem demo
  ...
  const canChanDangNhap = isLive && live.batBuocDangNhap && !user && !boChanTam;
  if (canChanDangNhap) {
    return <AuthGate onDemo={HAS_SUPABASE ? () => { setDataSource("demo"); setBoChanTam(true); } : null} />;
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: PAGE_BG, color: COLOR.ink, fontFamily: "'Inter','Montserrat',ui-sans-serif,system-ui,-apple-system,'Segoe UI',sans-serif" }}>
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 py-6">
        <header className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <button
              onClick={() => { if (!HAS_SUPABASE) { alert("Chưa cấu hình Supabase (VITE_SUPABASE_URL / ANON_KEY). Đang ở chế độ demo."); return; } setDataSource((s) => (s === "live" ? "demo" : "live")); }}
              className="flex items-center gap-2.5 rounded-2xl bg-white px-4 ring-1 h-[50px] transition"
              style={{ ...cardShadow, borderColor: isLive ? COLOR.teal : undefined }}
              title={HAS_SUPABASE ? "Bấm để đổi nguồn dữ liệu" : "Chưa cấu hình Supabase"}>
              <span className={`w-2.5 h-2.5 rounded-full ${isLive ? "bg-teal-400" : "bg-slate-300"} ${isLive && live.dangTai ? "animate-pulse" : ""}`} />
              <div className="leading-tight text-left"><p className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Nguồn dữ liệu</p><p className="text-xs font-semibold" style={{ color: isLive ? COLOR.teal : COLOR.ink }}>{isLive ? "LIVE · Supabase" : "DEMO · dữ liệu mẫu"}</p></div>
            </button>
            {isLive && live.sucKhoe && <SucKhoeChip sk={live.sucKhoe} />}
      ...
      {loginOpen && <LoginModal onClose={() => setLoginOpen(false)} onLogin={setUser} isLive={isLive} />}
    </div>
  );
}